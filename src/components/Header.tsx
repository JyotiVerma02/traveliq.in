...
export default function Header() {
  ...
  const servicesActive =
    isActive("/our-services/") || services.some(([, href]) => isActive(href));
  const navClass = (href: string) =>
    `site-nav-link${isActive(href) ? " is-active" : ""}`;

  return (
   
  <header className="site-header relative z-50 border-b border-[#10407A]/10 bg-white shadow-[0_2px_12px_rgba(16,64,122,0.05)]">
     <div className="mx-auto flex h-[74px] max-w-7xl items-center justify-between px-5 sm:px-8">
        <Link href="/" aria-label="TravelIQ home" className="shrink-0">
          <Image
            src="/logo.png"
            alt="TravelIQ"
            width={166}
            height={40}
            priority
            className="h-auto w-[142px] sm:w-[166px]"
          />
        </Link>
        <nav
        ...
        >
          ...
          <div className="group relative">
            <button
              className={`site-nav-link flex items-center gap-1${servicesActive ? " is-active" : ""}`}
            >
            ...
            <div className="service-menu">
              ...
              <Link
                className={`font-bold text-[#ee5326]!${isActive("/our-services/") ? " is-active" : ""}`}
                href="/our-services/"
              >
        ...
        <div className="hidden items-center gap-3 lg:flex">
          ...
          <Link href="/irctc-agent-registration/" className="brand-button">
            Become an Agent
          </Link>
          <a
        ...
        <button
          className="grid h-10 w-10 place-items-center rounded-xl border border-[#10407a]/15 text-[#10407a] lg:hidden"
      ...
      {open && (
        <div className="mobile-menu lg:hidden">
          <nav>
            ...
            <div className="ml-3 border-l border-[#10407a]/10 pl-2">
              {services.map(([name, href]) => (
                <Link
                  onClick={() => setOpen(false)}
                  key={href}
                  href={href}
                  className={`text-sm!${isActive(href) ? " is-active" : ""}`}
                >
                  {name}
                </Link>
              ))}
